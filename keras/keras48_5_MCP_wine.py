...
hist = model.fit(x_train, y_train, epochs=5000, batch_size=100, callbacks=[es, cp], validation_split=0.2, verbose=2)

model.save('/study/_save/ModelCheckPoint/keras48_5_save_model_wine.h5')
end_time = time.time() - start_time """

start_time = time.time()

#model = load_model('/study/_save/ModelCheckPoint/keras48_5_save_model_wine.h5')

model = load_model('/study/_save/ModelCheckPoint/keras48_5_MCP_wine.hdf5')
...
'''
걸린시간 :  43.996960163116455
loss :  3.021383762359619
accuracy :  0.6265305876731873

load model
걸린시간 :  0.8308303356170654
loss :  3.021383762359619
accuracy :  0.6265305876731873

check point
걸린시간 :  0.8359060287475586
loss :  1.0597666501998901
